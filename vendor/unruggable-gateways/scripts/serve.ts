...
// A simplified fetch handler for our custom gateway
async function customFetch(req: Request, rollup: NameWrapperRollup) {
  if (req.method !== 'POST') {
    return new Response('unsupported', { status: 405 });
  }
  const { sender, data: calldata } = await req.json();
  const data = await rollup.handleRead(calldata);
...
const gatewayOrRollup = await createGateway(args[0]);
const port = parseInt(args[1] || process.env.PORT || '') || 8000;

if (args.length > 2) {
  throw new Error(`unknown args: ${args.join(' ')}`);
}
if (unfinalized && !gatewayOrRollup.rollup?.unfinalized) {
  throw new Error('unfinalized not supported');
}

if (printCalls) {
  [
    gatewayOrRollup.rollup?.provider1,
    gatewayOrRollup.rollup?.provider2,
  ].forEach((p) => {
    if (p) {
      p.on('debug', (x) => {
...
if (gatewayOrRollup instanceof Gateway) {
  // gateway.commitDepth = 100;
  // gateway.allowHistorical = true;
  if (gatewayOrRollup.rollup instanceof TrustedRollup) {
    gatewayOrRollup.commitDepth = 0; // no need to keep expired signatures
  } else if (typeof commitDepth === 'number') {
    gatewayOrRollup.commitDepth = commitDepth;
  } else if (gatewayOrRollup.rollup.unfinalized) {
    gatewayOrRollup.commitDepth = 10;
  }
}
if (latestBlockTag) {
  if (gatewayOrRollup instanceof Gateway) {
    gatewayOrRollup.rollup.latestBlockTag = latestBlockTag;
  }
}
if (disableCache) {
  if (gatewayOrRollup instanceof Gateway) {
    gatewayOrRollup.disableCache();
  }
}

// how to configure prover
if (gatewayOrRollup instanceof Gateway) {
  gatewayOrRollup.rollup.configure = (
    c: RollupCommitType<typeof gatewayOrRollup.rollup>
  ) => {
...
const config = toJSON({
  ...
  gateway: gatewayOrRollup.constructor.name,
  rollup: gatewayOrRollup.rollup?.constructor.name,
  unfinalized: gatewayOrRollup.rollup?.unfinalized,
  chain1: gatewayOrRollup.rollup
    ? chainDetails(gatewayOrRollup.rollup.provider1)
    : undefined,
  chain2: gatewayOrRollup.rollup
    ? chainDetails(gatewayOrRollup.rollup.provider2)
    : undefined,
  since: new Date(),
  supportsV1: gatewayOrRollup.rollup
    ? supportsV1(gatewayOrRollup.rollup)
    : false,
  supportsV2: gatewayOrRollup instanceof Gateway,
  prefetch,
  callCacheSize:
    gatewayOrRollup instanceof Gateway ? gatewayOrRollup.callLRU.max : 0,
  ...toJSON(gatewayOrRollup),
  ...Object.fromEntries(
    await Promise.all(
      Object.entries(gatewayOrRollup.rollup || {}).map(async ([k, v]) => {
...
if (gatewayOrRollup.rollup instanceof TrustedRollup) {
  config.signer = gatewayOrRollup.rollup.signerAddress;
}
...
if (dumpAndExit) {
  ...
  const t0 = Date.now();
  const commit =
    gatewayOrRollup instanceof Gateway
      ? await gatewayOrRollup.getLatestCommit()
      : null;
  if (commit) {
...
if (prefetch) {
  ...
  const fire = async () => {
    try {
      if (gatewayOrRollup instanceof Gateway) {
        const t0 = Date.now();
        const commit = await gatewayOrRollup.getLatestCommit();
        console.log(
          new Date(),
    ...
    if (gatewayOrRollup instanceof Gateway) {
      setTimeout(fire, gatewayOrRollup.latestCache.cacheMs);
    }
  };
  await fire();
}
...
export default {
  port,
  async fetch(req) {
    // If we created our custom rollup, use a simplified handler
    if (gatewayOrRollup instanceof NameWrapperRollup) {
      return customFetch(req, gatewayOrRollup);
    }

    // Original gateway logic
    const gateway = gatewayOrRollup;
    switch (req.method) {
      case 'OPTIONS': {
      ...
      case 'POST': {
        ...
        try {
          // If we are running our custom rollup, use its simplified handler
          if (gatewayOrRollup instanceof NameWrapperRollup) {
            const { data: calldata } = await req.json();
            const data = await gatewayOrRollup.handleRead(calldata);
...
async function createGateway(name: string): Promise<any> {
  // Return type changed to any
  if (name === 'comet-ens') {
    console.log('Creating custom CometENS Rollup...');
    ...
    return new NameWrapperRollup(l2RpcUrl, nameWrapperAddress);
  }
